...
app.post('/api/assistant/chat', async (req, res) => {
  try {
    ...
    let reply = 'I am the VEMU Library Assistant. Please ask about books, users, transactions, renewals, suggestions, or feedback.';

    if (message.includes('hello') || message.includes('hi')) {
      reply = 'Hello! Welcome to VEMU Library Assistant. How can I help you today?';
    } else if (message.includes('book')) {
      reply = 'You can search books, issue books, return books, or send renewal requests from the library system.';
    } else if (message.includes('renewal')) {
      reply = 'Students and faculty can send renewal requests. Admin and librarian can review and approve them.';
    } else if (message.includes('fine')) {
      reply = 'Fines are calculated based on overdue or damage details available in the system.';
    } else if (message.includes('login')) {
      reply = 'Please use your assigned user ID and password to log in to the VEMU Library portal.';
    } else if (message.includes('suggestion')) {
      reply = 'You can submit book suggestions from your dashboard. Admin and librarian can review them.';
    }
